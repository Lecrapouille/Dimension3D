...
namespace dim
{
	...
	void DragController::check_events(const std::optional<sf::Event>& sf_event, Scene& scene, Camera& camera)
	{
		...
		{
			float delta = static_cast<float>(mouseWheelScrolled->delta);
			camera.position += normalize(camera.direction) * speed * delta;

			if (camera.get_type() == Camera::Type::Orthographic)
				static_cast<OrthographicCamera&>(camera).zoom(1.f + delta * speed * 0.5f);
			...
			scene.camera2D.zoom(1.f - (speed * delta * 0.5f));
	...
	void DragController::check_events(const std::optional<sf::Event>& sf_event, Camera& camera)
	{
		const auto* mouseWheelScrolled = sf_event->getIf<sf::Event::MouseWheelScrolled>();
		if (move_active && mouseWheelScrolled && Window::is_in(sf::Mouse::getPosition(Window::get_window())))
		{
			...
			Window::camera2D.zoom(1.f - (speed * delta * 0.5f));
